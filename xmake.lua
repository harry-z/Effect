...
target("EffectMain")
    ...
    add_includedirs("Libs/jpeg", "Libs/stb/include", "Libs/assimp/include")
    add_linkdirs("Libs/assimp/$(plat)/$(arch)/$(mode)")
...
target("Effect")
    ...
    after_link(
        function (target)
            print("copying shaders and resources ...")
            os.mkdir("$(buildir)/$(plat)/$(arch)/$(mode)/Shaders")
            os.mkdir("$(buildir)/$(plat)/$(arch)/$(mode)/Resources")
            os.cp("$(projectdir)/Shaders/*.*", "$(buildir)/$(plat)/$(arch)/$(mode)/Shaders")
            os.cp("$(projectdir)/Resources/*.*", "$(buildir)/$(plat)/$(arch)/$(mode)/Resources")
            -- copy assimp
            print("copying third party libraries ...")
            os.cp("$(projectdir)/Libs/assimp/$(plat)/$(arch)/$(mode)/*.dll", "$(buildir)/$(plat)/$(arch)/$(mode)")
            os.cp("$(projectdir)/Libs/assimp/$(plat)/$(arch)/$(mode)/*.pdb", "$(buildir)/$(plat)/$(arch)/$(mode)")
            print("finish.")
        end
    )
target_end()
