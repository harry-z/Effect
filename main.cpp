...
int _tmain(int argc, TCHAR** argv)
{
    ...
    while (!bQuit)
    {
        ...
        do
        {
            if (nSelection > g_GlobalData.m_vecEffects.size())
                std::cout << "Typed an invalid effect number." << std::endl;
            std::cout << "Please select an effect, or input '0' to quit: " << std::endl;
            for (size_t i = 0, iEnd = g_GlobalData.m_vecEffects.size(); i < iEnd; ++i)
            {
                std::cout << i + 1 << ". " << g_GlobalData.m_vecEffects[i].c_str() << std::endl;
            }          
            std::cout << "Your selection: ";
            std::cin >> nSelection;
        } while (nSelection > 0 && nSelection <= g_GlobalData.m_vecEffects.size());
        ...
        String szLibName = g_GlobalData.m_vecEffects[nSelection] + _T(".dll");
